...
* Right now we are pulling in two directions. The first is using the meetupr package to identify some information about popular groups, or if possible, specific popular events in an area where we manually imput the area. The second is creating a package like "meetupr"" that accesses meetup's rsvpticker data and generates a dataframe from the rsvp's as they appear. * 
...
paris <- as.data.frame(df) %>% 
  select(group_name,venue.name, name) %>% 
  group_by(group_name) %>% 
  filter(venue.name =="Metro Anvers" | venue.name == "brasserie le st andré") %>% 
  summarise(venue.name, count = n())
paris1<-as.data.frame(df)
df$venue.name

```

The paris data frame shows the number of rsvps made in two locations, Metro Anvers and brasserie lest andré. So far, this is not working because the venue.name is null.