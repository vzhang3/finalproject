...
bo <- read_csv("data/MA.csv", col_names= FALSE,numerals=)
...
column_rename(x = ch)
column_rename(x = ha)
#doesn't work but I'll look at that later
...
topcatbygroup<-function(x){ggplot(data=x, aes(x=reorder(category, group_tot), y=group_tot))+geom_bar(stat="identity", aes(fill=group_tot)) +coord_flip()+ylab("Total groups")+xlab("Category")+scale_fill_continuous(name="top groups per category")
}
